...
// TODO(@s0cks): make compatible w/ gel heap
namespace gel {
ArrayBase::ArrayBase(const word init_cap) {
  if (init_cap > 0) {
    const auto new_cap = RoundUpPow2(init_cap);
    const auto address = sys::malloc(sizeof(uword) * new_cap); 
...
void ArrayBase::Resize(const word new_length) {
  if (std::cmp_greater(new_length, capacity_)) {
    ...
    const auto new_data = sys::realloc((uword)data_, sizeof(uword) * new_cap); 
...
auto ArrayBase::Compare(Value* rhs) const -> std::strong_ordering {
  ASSERT(rhs);
