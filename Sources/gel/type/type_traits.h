...
//
// template <typename T>
// concept StringLike = std::convertible_to<T, std::string> || std::same_as<T, gel::Str>;
// //
// template <typename T>
...
//
template<typename T>
concept HasOwner = requires(T value) {
  { value.GetOwner() } -> std::convertible_to<gel::Value*>;
  { value.HasOwner() } -> std::convertible_to<bool>;
};
...
template<typename T>
concept HasDocs = requires(T value) {
  { value.GetDocs() } -> std::convertible_to<gel::Str*>;
  { value.HasDocs() } -> std::convertible_to<bool>;
};
