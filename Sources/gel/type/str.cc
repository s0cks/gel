...
namespace gel {
auto Str::GetHashCode() const -> HashCode {
  HashCode hash{};
  // hash ^= value_;
  return hash;
}

auto Str::Compare(Value* rhs) const -> std::strong_ordering {
  ASSERT(rhs);
  if(!rhs || !rhs->IsStr())
    return std::strong_ordering::greater;
  return value_ <=> rhs->AsStr()->str();
}

auto Str::ToString() const -> std::string {
  ToStringHelper<Str> helper{};
  helper.AddField("value", value_);
  return helper;
}
//
