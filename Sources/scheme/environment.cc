...
namespace scm {
auto Environment::Put(const Key& k, Value* v) -> bool {
  ASSERT(!k.empty());
  ASSERT(v);
...
auto Environment::Has(const Key& k) const -> bool {
  ASSERT(!k.empty());
  auto env = this;
  while (env != nullptr) {
...
auto Environment::LocalLookup(const Key& k, Value** result) const -> bool {
  ASSERT(!k.empty());
...
auto Environment::Lookup(const Key& k, Value** result) const -> bool {
  auto current = this;
