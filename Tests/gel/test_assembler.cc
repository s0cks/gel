...
#define COMPILE(Name)               \
  CompiledCode Name(__ Assemble()); \
  ASSERT_TRUE((Name).IsCompiled());

static inline auto OperandAddress(const CompiledCode& code) -> uword {
...
static inline auto OperandArgs(const CompiledCode& code) -> uword {
...
static inline auto OperandValue(const CompiledCode& code) -> word {
